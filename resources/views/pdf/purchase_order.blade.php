...
<head>
    <meta charset="UTF-8">
    <title>Purchase Order - {{ $po->referensi_PO }}</title>
    <style>
...
<body>
    ...
    <div class="header">
        ...
        <p>Universal Furniture &mdash; Dokumen Pemesanan Barang</p>
    </div>
    ...
    <table class="info-grid">
        <tr>
            <td>
                <div class="info-label">No. Referensi PO</div>
                <div class="info-value">{{ $po->referensi_PO }}</div>
            </td>
            <td>
                <div class="info-label">Tanggal PO</div>
                <div class="info-value">
                    {{ \Carbon\Carbon::parse($po->tgl_PO)->translatedFormat('d F Y') }}
                </div>
            </td>
            <td>
                <div class="info-label">Dari PR</div>
                <div class="info-value">
                    {{ $po->purchaseRequest?->referensi_PR ?? '-' }}
                </div>
            </td>
            <td>
    ...
    {{-- ── Supplier ── --}}
    @php $supplier = $po->details->first()?->supplier; @endphp
    @if($supplier)
    ...
    <table class="items">
        ...
        <tbody>
            ...
            @foreach($po->details as $i => $detail)
                @php
                    $subtotal   = $detail->hargabarangPO * $detail->kuantitasbarangPO;
                    $grandTotal += $subtotal;
                @endphp
                <tr>
                    <td>{{ $i + 1 }}</td>
                    <td>{{ $detail->barang->nama_barang ?? '-' }}</td>
                    <td class="right">Rp {{ number_format($detail->hargabarangPO, 0, ',', '.') }}</td>
                    <td class="right">{{ $detail->kuantitasbarangPO }} unit</td>
                    <td class="right">
                        {{ $detail->kuantitasterimaPO !== null ? $detail->kuantitasterimaPO . ' unit' : '-' }}
                    </td>
                    <td>
                        {{ $detail->tgl_terima
                            ? \Carbon\Carbon::parse($detail->tgl_terima)->translatedFormat('d F Y')
                            : '-' }}
                    </td>
